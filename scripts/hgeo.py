...
def _unpackPrimRLE(rle):
    ''' Unpack a run-length encoded array of bit data (used to save groups) '''
    a = []
    for run in range(0, len(rle), 2):
        a += [rle[run]] * rle[run+1] # state * count
    return a
...
class ElementGroup:
    ...
    def loadSelection(self, obj, element_count):
        ...
        self.Defaults = sel.get('defaults', None) #sel['defaults']
...
def savePolyRun(prim):
    return [
        "startvertex", prim.StartVertex,
        "nprimitives", prim.nprimitives,
        "nvertices_rle", prim.nvertices_rle
    ]
...
primSavers = {
    ...
    'Tube'        : saveTube,
    'Volume'      : saveVolume,
    'Polygon_run' : savePolyRun
}

class PolyRun:
    def __init__(self, start_vertex=0, nprimitives = 0, nvertices_rle = []):
        self.StartVertex = start_vertex
        self.nprimitives = nprimitives
        self.nvertices_rle = nvertices_rle
    ...
    def save(self):
        return [
            ...
            "nvertices_rle", self.nvertices_rle,
        ]
...
class Detail:
    ...
    def loadSinglePrimitive(self, pdef, pdata):
        ...
        ptype = pdef['type']
        if ptype == 'Polygon_run':
            start_vertex, nprimitives, polygon_sizes, nvertices_rle = primRun(listToDict(pdata))
            cur_vert = start_vertex
            for p in range(len(polygon_sizes)):
                verts = []
                for v in range(cur_vert, cur_vert + polygon_sizes[p]):
                    verts.append(v)

                prim = Primitive(ptype, verts)
                self.Primitives.append(prim)
                cur_vert += polygon_sizes[p]

            self.PolyRuns.append(PolyRun(start_vertex, nprimitives, nvertices_rle))
        else:
    ...
    def savePrimitives(self, polyruns : bool, primitives : bool):
        ''' Create the JSON schema for all the primitives '''
        output = []

        if primitives:
            for p in self.Primitives:
                output.append(p.save())

        if polyruns:
            for p in self.PolyRuns:
                output.append(
                    [
                        ["type", "Polygon_run"],
                        p.save()
        ...
        return ["primitives", output]#[ "primitives", prims ]
    ...
    def saveJSON(self):
        ...
        data += self.savePrimitives(polyruns=True, primitives=False)
    ...
    def save(self, fp, indent="    "):
        ''' Save the JSON schema to a file '''
        with open(fp, 'w') as f:
            data = hjson.dumpsJSON(self.saveJSON(), f, indent=indent)
            f.write(data)
